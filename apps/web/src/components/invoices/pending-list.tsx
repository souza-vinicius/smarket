...
export function PendingList({ items, isLoading = false, onDelete, isDeleting = false }: PendingListProps) {
    ...
    return (
      <div className="space-y-4 p-8">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={`skeleton-${String(i)}`} className="h-20 animate-pulse rounded-lg bg-slate-100" />
        ))}
      </div>
    );
  }
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-2 flex items-center gap-2">
                <h3 className="truncate font-semibold text-slate-900">
                  {item.extracted_issuer_name || `Processamento ${item.processing_id.slice(0, 8)}...`}
                </h3>
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (onDelete) onDelete(item.processing_id);
                }}
                disabled={isDeleting}
              >

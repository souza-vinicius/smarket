...
class PurchasePattern(Base):
    ...
    frequency: Mapped[Optional[Frequency]] = mapped_column(
        SQLEnum(Frequency), nullable=True
    )
    # Média de dias entre compras
    average_interval_days: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    alert_threshold_days: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    # Relationships
    user: Mapped["User"] = relationship(back_populates="purchase_patterns")

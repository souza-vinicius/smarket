...
async def get_invoice_item(
    ...
    result = await db.execute(
        ...
        .where(and_(InvoiceItem.id == item_id, Invoice.user_id == current_user.id))
    )
...
async def bulk_categorize(
    ...
    cat_result = await db.execute(
        select(Category).where(
            and_(
                Category.id == request.category_id, Category.user_id == current_user.id
            )
        )
    )
    category = cat_result.scalar_one_or_none()
    ...
    return {"updated_count": result.rowcount, "category_id": request.category_id}
